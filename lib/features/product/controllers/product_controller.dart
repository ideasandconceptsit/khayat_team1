...
class ProductCardController extends GetxController {
  ...
  void addItemToCart() async {
    try {
      ...
      var result = await productRepository.addItemToCart(
          product.id, product is FabricProductModel ? "Fabric" : "Accessories");
  ...
  void onInit() {
    ...
    if (product.colors?.isNotEmpty ?? false) {
      pageController = PageController(
          initialPage: product.colors!.indexOf(selectedProductColor.value));
    }
    else {
      pageController = PageController();
    }
